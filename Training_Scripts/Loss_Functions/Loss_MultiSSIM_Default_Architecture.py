...
class Configuration:
    checkpoint_file_name = 'loss_MultiSSIM_checkpoint'  # The number will follow and then the .pt
    load_model_file_name = 'loss_MultiSSIM_checkpoint18.pt'
    starting_epoch = 1  # + 13
...
ssim = MultiScaleStructuralSimilarityIndexMeasure().to(config.device)
loss_criterion = lambda preds, target: 1 - ssim(preds, target)  # as its one when equal

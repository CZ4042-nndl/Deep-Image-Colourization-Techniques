...
class Configuration:
    ...
    model_file_name = 'checkpoint14.pt' #TODO: change your file name if load_model_to_train = True

    load_model_to_train = True #TODO: True, continue training | False, train with init_weights
    next_epoch = 15 #TODO: set as (latest checkpoint number + 1) if load_model_to_train = True
...
# normalize=True since values are [0, 1]
...
if not config.load_model_to_test:
    ...
    for epoch in tqdm(range(start, hparams.epochs)):
        ...
        for idx,(img_l_encoder, img_rgb_encoder, img_l_inception, file_name) in enumerate(train_dataloader):
            #*** Skip bad data ***
            ...
            output_rgb = model(img_l_encoder,img_embs)

            #*** Back propogation ***
            # loss = loss_criterion(output_ab, img_ab_encoder.float())
            pred = output_rgb
            target = img_rgb_encoder.float()

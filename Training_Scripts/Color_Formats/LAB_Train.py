...
class Configuration:
    ...
    load_model_to_train = True #TODO: True, continue training | False, train with init_weights
    model_file_name = 'checkpoint18.pt' #TODO: change your file name if load_model_to_train = True
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            l = torch.stack([torch.Tensor(l_encoder_img)])

            #*** Normalize a and b channels and concatenate with l ***
            ...
            lab_encoder_norm_img = torch.cat([a_encoder_img, b_encoder_img, l], dim=0)
            ...
            return l_encoder_img, lab_encoder_norm_img, l_inception_img, self.files[index]
...
if not config.load_model_to_test:
    ...
    for epoch in tqdm(range(start, hparams.epochs)):
        ...
        for idx,(img_l_encoder, img_lab_encoder, img_l_inception, file_name) in enumerate(train_dataloader):
            #*** Skip bad data ***
            ...
            img_lab_encoder = img_lab_encoder.to(config.device)
            ...
            output_lab = model(img_l_encoder,img_embs)

            #*** Back propogation ***
            loss = loss_criterion(output_lab, img_lab_encoder.float())
            loss.backward()

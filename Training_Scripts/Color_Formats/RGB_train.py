...
class Configuration:
    ...
    model_file_name = 'checkpoint17.pt' #TODO: change your file name if load_model_to_train = True
    next_epoch = 18 #TODO: set as (latest checkpoint number + 1) if load_model_to_train = True
...
if not config.load_model_to_test:
    ...
    for epoch in tqdm(range(start, hparams.epochs)):
        ...
        for idx,(img_l_encoder, img_rgb_encoder, img_l_inception, file_name) in enumerate(train_dataloader):
            ...
            #*** Back propogation ***
            loss = loss_criterion(output_rgb, img_rgb_encoder.float())
            loss.backward()
